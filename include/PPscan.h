...
public:
    PPscan(const unordered_map<int, set<int>>& homoGraph, const map<int, vector<int>>& hinGraph, const vector<int>& vertexType, const vector<int>& edgeType, const unordered_map<int, int>& edgeReverseMap, const MetaPath& metaPath, int mode);
    ~PPscan();

    void get_graph();
    void pSCAN3(const char* eps_s, int miu); // pSCAN3 + pSCAN_disjoint3 = parallel disjoint situation
    void pSCAN_disjoint3();
...
private:
    ...
    int disjoinNb(const vector<int>& commonNB, int vertexU, int vertexV, int c);
